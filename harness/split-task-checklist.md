...
或 `verify <slug>` 时，executor 会自动为 `src/frontend/node_modules` 建立指向主仓的软链
（日志前缀 `[worktree-deps]`）。若主仓也没有对应目录会打印告警——此时需先回主仓执行
`pnpm install` / `make build` 产出依赖再回 worktree。

**并发保护设计**：为支持多 worktree 并行 build/verify，以下资源**每个 worktree 独立**，
不与主仓或其他 worktree 共享：

- `src/backend/target/`：Maven 构建产物，worktree 独立，避免多个 `mvn package` 互踩
- `src/frontend/.vite-cache/`：vite dev/build 缓存（通过 `vite.config.js` 的 `cacheDir` 显式配置）
- `src/frontend/.vitest-cache/`：vitest 测试缓存（通过 `test.cache.dir` 显式配置）

**只有 `src/frontend/node_modules` 是软链到主仓的共享只读依赖**，因为包体积大且运行时不写入。
...
# 注意：不要软链 src/backend/target，保持 worktree 独立
```
...
## 影响范围
- 受影响文件：
  - `src/backend/src/main/java/com/xptsqas/api/PvController.java`（删除）
  - `src/backend/src/main/java/com/xptsqas/api/pv/`（新增子包）
